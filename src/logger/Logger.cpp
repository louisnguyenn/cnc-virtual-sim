...
void Logger::writeToCSV(const std::string& path) const
{
    ...
    {
        auto ms =
            std::chrono::duration_cast<std::chrono::milliseconds>(record.timestamp - m_startTime)
                .count();

        csv_file << ms << ','; // write timestamp

        csv_file << record.pos.x << ',' << record.pos.y << ',' << record.pos.z
                 << ','; // write position
...
{
    std::cout << "\r[" << status << "] X:" << pos.x << " Y:" << pos.y << " Z:" << pos.z
              << " F:" << feedrate << "          " << std::flush;
}

void Logger::printSummary(const MachineState& state) const
{
    std::cout << "\n========= SUMMARY =========\n";
    std::cout << "Final position  : X=" << state.position.x << " Y=" << state.position.y
              << " Z=" << state.position.z << "\n";
